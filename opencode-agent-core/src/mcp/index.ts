...
export namespace MCP {
        ...
        try {
          ...
          if (isAuthError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              // Bus.publish(TuiEvent.ToastShow, { ... }) — TUI removed
              log.warn("MCP auth required", {
                key,
  ...
  export async function authenticate(mcpName: string): Promise<Status> {
    ...
    try {
      // open(authorizationUrl) removed — browser opening not applicable for server-side agent
      // In headless mode, emit event so external systems can handle the auth URL
      log.warn("browser opening not available in headless mode, user must open URL manually", { mcpName })
